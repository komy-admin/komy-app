...
function buildMenuData(
  ...
  Object.entries(selections).forEach(([categoryId, selectionsArray]) => {
    ...
    for (const selection of selectionsArray) {
      const menuCategoryItem = category.items?.find(
        (mi) => mi.item?.id === selection.itemId,
      );
      if (!menuCategoryItem?.item) continue;

      const tagsPrice = selection.tags.reduce(
        (sum, t) => sum + (t.priceModifier || 0),
        0,
      );
      ...
      menuItems.push({
        ...
        categoryId,
        categoryName:
          itemTypes.find((t) => t.id === category.itemTypeId)?.name || "",
        status: Status.DRAFT,
        item: {
...
function convertToApiFormat(lines: OrderLine[]): CreateOrderLineRequest[] {
  return lines.map((line): CreateOrderLineRequest => {
    if (line.type === OrderLineType.ITEM) {
      ...
      > = {};
      const menuItems =
        (line.items as DraftMenuItemWithMeta[] | undefined) || [];
...
export const useOrderLinesManager = (options: UseOrderLinesManagerOptions) => {
  ...
  const [initialOrderLines, setInitialOrderLines] =
    useState<OrderLine[]>(initialLines);
  ...
  const addItem = useCallback(
    (item: Item, customization: { tags: SelectedTag[]; note?: string }) => {
      const tagsPrice = customization.tags.reduce(
        (sum, t) => sum + (t.priceModifier || 0),
        0,
      );
  ...
  const updateItem = useCallback(
    (lineId: string, customization: { tags: SelectedTag[]; note?: string }) => {
      setOrderLines((prev) =>
        prev.map((line) => {
          if (line.id !== lineId || line.type !== OrderLineType.ITEM)
            return line;
          if (!line.id.startsWith("draft-") && line.status !== Status.DRAFT)
            return line;
  ...
  const addMenu = useCallback(
    (menu: Menu, selections: MenuSelections, itemTypes: any[]) => {
      const { items, totalPrice, now } = buildMenuData(
        menu,
        selections,
        itemTypes,
      );
      ...
      const newLine: OrderLine = {
        ...
        totalPrice: totalPrice,
        status: Status.DRAFT,
        // Champs de paiement avec valeurs par défaut
        paidAmount: 0,
        paidFraction: 0,
        paymentStatus: "unpaid",
        item: null,
  ...
  const updateMenu = useCallback(
    (
      ...
      setOrderLines((prev) =>
        prev.map((line) => {
          if (line.id !== lineId || line.type !== OrderLineType.MENU)
            return line;

          const { items, totalPrice } = buildMenuData(
            menu,
            selections,
            itemTypes,
          );
