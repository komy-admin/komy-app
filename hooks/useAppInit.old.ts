...
import { sessionActions, selectAppInitialized, selectIsAppInitializing } from '~/store/slices/session.slice';
...
interface InitializationState {
  ...
  isLoading: boolean;
  isInitializing: boolean; // Flag pour éviter les doubles appels
  hasEverInitialized: boolean; // Flag pour éviter la sync prématurée
  error: string | null;
...
export const useAppInit = () => {
  ...
  // Check if user is properly authenticated (either with sessionToken or authToken + PIN verified)
  const canInitialize = !!(
    (sessionToken && user) ||
  ...
  const initializeApp = useCallback(async () => {
    ...
    // Éviter les doubles initialisations avec le flag local
    if (state.isInitialized || state.isLoading || state.isInitializing || state.error) {
    ...
    // IMPORTANT: Marquer immédiatement dans Redux pour éviter les triples requêtes
    ...
    setState(prev => ({
      ...prev,
      isInitialized: false,  // Force false to display the loader (flag local différent!)
      isLoading: true,
      isInitializing: true,  // Flag local pour éviter les doubles appels
      hasEverInitialized: true, // Marquer qu'on a initialisé pour éviter sync prématurée
      error: null
    }));
    ...
    try {
      ...
      // Marquer comme terminé (flag local seulement - Redux déjà fait)
      ...
      setState(prev => ({
        ...
        isLoading: false,
        isInitialized: false,
        isInitializing: false, // Réinitialiser le flag pour permettre une nouvelle tentative
        hasEverInitialized: false, // Réinitialiser pour permettre une nouvelle tentative
        error: errorMessage
      }));
  ...
  useEffect(() => {
    ...
    } else if (canInitialize && user?.profil && appInitialized && !state.isInitialized && !state.isLoading && !state.isInitializing && !state.error && !state.hasEverInitialized) {
  ...
  }, [canInitialize, user?.profil, user?.id, appInitialized, isPinVerified, dispatch]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // Debug temporaire
  console.log('🔍 [useAppInit] State retourné:', {
    isInitialized: state.isInitialized,
    isLoading: state.isLoading,
    isInitializing: state.isInitializing,
    error: state.error,
    appInitializedRedux: appInitialized
  });
